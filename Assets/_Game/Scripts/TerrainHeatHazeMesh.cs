...
{
    /// <summary>
    /// Generates a thin mesh that hugs the terrain surface, used as the
    /// carrier for the heat haze distortion shader. Vertex alpha stores a
    /// soft mask that fades the effect out on high ground and steep slopes.
    /// </summary>
    ...
    public class TerrainHeatHazeMesh : MonoBehaviour
    {
        ...
        public void Generate()
        {
            ...
            // drop the previously generated mesh so editor regeneration does
            // not pile up orphaned meshes in memory
            if (previous != null && previous.name == mesh.name)
            {
#if UNITY_EDITOR
                if (!UnityEditor.EditorUtility.IsPersistent(previous))
                {
